...
class ClothDetector:
    ...
    def enforce_torso_region(self, mask):
        """
        Restrict mask to a central torso strip; removes expansion
        to arms / lap / background when SegFormer over-segments.
        """
        h, w = mask.shape[:2]

        # Keep only central 40%–60% width (tune if needed)
        x1 = int(w * 0.30)
        x2 = int(w * 0.70)

        torso = np.zeros_like(mask)
        torso[:, x1:x2] = mask[:, x1:x2]
